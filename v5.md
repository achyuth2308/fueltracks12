...
Location packet UTC time
## 0x22
## Heartbeat Packet 0x13
Response of GPRS command  0x21
Alarm Data（UTC）
## 0x26
## Online Command 0x80
...
again.
lIf no packet returned by server within 5 seconds, then the response of login packet is timeout.
lTerminal reboot automatically after 3 timeouts.
...
- Login packet
## Description：
l Login packet is the information packet connecting the terminal and platform. It can send terminal
information to platform.
l If a GPRS connection is established successfully, the terminal will send a first login message packet to
the server and, within five seconds, if the terminal receives a data packet responded by the server, the
connection is considered to be a normal connection; if not, the terminal will send login packet
...
, are  values  of  CRC-ITU.  CRC  error  occur  when  the  received
...
2.1.Heartbeat packet sent by device to server
## Heartbeat Packet

## 2. Heartbeat Packet
## Description：
a) Heartbeat packet is a data packet to maintain the connection between the device  and the server.
b) If a GPRS connection is established successfully, the terminal will send a first login message packet to the
server and, within five seconds, if the terminal receives a data packet responded by the server, the connection
is considered to be a normal connection; if not, the terminal will send login packet again.
...
2.2Server response from server to heartbeat packet
...
function) divided by 1000 (for meter to Km.)
## Serial Number 2
Serial number of data sent later at each time will be automatically
added ‘1’.
...
3.2 location packet response from Server
Location packet response is not mandatory

Note :  if PBSW is on then only response is required from server for history
location packet: because after enabling the “pbsw,on” location packet
required ACK from server: “which hbt packet response, after enabling the
psbw device send the history packet with hbt packet ”

example：78 78 22 22 0F 0C 1D 02 33 05 C9 02 7A C8 18 0C 46 58 60 00 14 00 01 CC 00 28 7D 00 1F 71 00
00 01 00 08 20 86 0D 0A 78 78 22 22 0F 0C 1D 02 33 05 C9 02 7A C8 18 0C 46 58 60 00 14 00 01 CC 00 28 7D
00 1F 71 00 00 01 00 08 20 86 0D 0A 78 78 22 22 0F 0C 1D 02 33 05 C9 02 7A C8 18 0C 46 58 60 00 14 00 01 CC
00 28 7D 00 1F 71 00 00 01 00 08 20 86 0D 0A 78 78 22 22 0F 0C 1D 02 33 05 C9 02 7A C8 18 0C 46 58 60 00 14
00 01 CC 00 28 7D 00 1F 71 00 00 01 00 08 20 86 0D 0A 78 78 22 22 0F 0C 1D 02 33 05 C9 02 7A C8 18 0C 46
58 60 00 14 00 01 CC 00 28 7D 00 1F 71 00 00 01 00 08 20 86 0D 0A 78 78 0A 13 40 04 04 00 01 00 0F DC EE
## 0D 0A

response - 78 78 05 13 01 00 E1 A0 0D 0A
## Bit0
BYTE_2 Bit5 0
BYTE_2 Bit4 0
BYTE_2 Bit3 1
...
0x02：Very Low Battery (Low Battery Alarm) 0x03：
Low Battery (can be used normally) 0x04：Medium 0x05
：High
0x06：Full
GSM Signal
## Strength
...
0x14 Door alarm (for GT800 only)
...
5.1 Online command sent by server
...
5.2 Online command replied by terminal
5.2.1 Terminal reply（general command: V5/VL149/GT800）
...
Information Serial 2 Serial number of data sent later at each time will be automatically
...
6.1 Time request sent by terminal
...
6.2 Server response time information
...
7 Information transmission packet
## Description：
Terminal transmits all types of non-position data.

7.1 Information transmission packet sent by terminal
...
00 External power voltage 01~03（
...
## 7.2 Server Response Information Transmission Packet
...
Data length behind start position of split-transmitting
...
send login data packet to server less than 20min, reconnect reconnection time?
...
raw data : 78 78 26 22 19 04 15 06 19 12 CB 00 F8 D0 E9 08 36 37 CC 00 14 00 01 94 5F B7 34 00 E5 3B
## 01 0E 00 00 1B 88 19 03 7E 8C 88 0D 0A
## ↆ
Package Type :  Positioning package (22)
Packet length : 38(26)
time =25/4/21 6:25:18(190415061912)
GPS Message length is 12, The number of satellites involved in positioning is 11 Pieces (CB)
latitude=N 9.05911611111111°(00F8D0E9),longitude=E 76.53942°(083637CC),speed : 0KM/H(00)
Positioning mode : Real time positioning , Positioning status : Located , course :0°(1400)
MCC:404(0194),MNC:95(5F),LAC:46900(B734),Cell ID:58683(00E53B)
ACC high (01), Reporting mode :GPSDUP upload ( Timed upload under static state )(0E),Real time
upload (00)
There are different solutions for locating packages, Total mileage :1804313 M(001B8819))
Verification information : 8C88(8C88)


Login packet example with response from server :

raw data : 78 78 11 01 08 67 01 00 70 11 34 52 80 8D 21 21 00 91 F3 C4 0D 0A
## ↆ
Package Type :  Login package (01)
Packet length : 17(11)
IMEI number : 867010070113452(0867010070113452)
type of machine : EV49G (808D)
time zone :  east 5.3 ,language : Chinese (01) (2121)
Message serial number : 0091(0091)
Verification information : F3C4(F3C4)




<=============================== server response sample of login pack=============================>
raw data : 78 78 05 01 00 91 4D 55 0D 0A
## ↆ
Package Type :  Login package server reply (01)
Packet length : 5(05)
Message serial number : 0091(0091)
Verification information : 4D55(4D55)

note Srial number will same as login packet (copy from login packet and use in login packet
response, logic is same for heartbeat packet also.)
...
Heart beat packet example with server response :

raw data : 78 78 0A 13 00 05 04 00 02 00 F5 A8 72 0D 0A
## ↆ
## Package Type :  Heartbeat  (13)
Packet length : 10(0A)
Oil electricity connection (03 undefined ), GPS Not located , No upload alarm , normal (GK309),
Remote locking : no , Charger not connected , ACC low , Turn off defences ( Unlock )  (00)
Voltage level :  High power (05)   GSM signal intensity  :  Signal strength (04)   language :
english (0002)
Message serial number : 00F5(00F5)
Verification information : A872(A872)


<=============================== Heartbeat packet response sample ==========================>
raw data : 78 78 05 13 00 F5 58 5A 0D 0A
## ↆ
Package Type :  Heartbeat packet server reply (13)
Packet length : 5(05)
Message serial number : 00F5(00F5)
Verification information : 585A(585A)
...
raw data : 78 78 25 26 19 04 12 0C 18 38 CF 01 DE EF 95 08 68 78 AA 00 14 6A 09 01 94 31 4F 2E 00 91
## DF 50 06 04 02 02 00 14 E9 7A 0D 0A
## ↆ
Package Type :  Upload alarm data package (26)
Packet length : 37(25)
time :25/4/18 12:24:56(1904120C1838)
GPS Message length is 12, The number of satellites involved in positioning is 15 Pieces (CF)
latitude :N 17.4375227777778°(01DEEF95),longitude :E 78.36909°(086878AA),speed : 0KM/H(00)
Positioning mode : Real time positioning , Positioning status : Located , course :106°(146A)
LBS length :9(09)   MCC:404(0194),MNC:49(31),LAC:20270(4F2E),Cell ID:37343(0091DF)
Main Power disconnect upload alarm  ,Oil electricity connection  ,Located  ,Main power not
connected  ,ACC low  ,Turn off defences (50)
Voltage level : Very high charge  (06),GSM Signal level : Signal strength  (04),
Alarm Type:Power off upload alarm  (002),english (2)
Message serial number : 0014(0014)
Verification information : E97A(E97A)
...
External Voltage (vehicle battery voltage ) sample packet

raw data : 79 79 00 08 94 00 05 06 02 E6 03 27 0D 0A
## ↆ
Package Type :  Data transmission general package (94)
Packet length : 8(0008)
Information Type :  External voltage (00) ,Transfer file information :0506(0506)
External voltage =12.86(0506)
Message serial number : 02E6(02E6)
Verification information : 0327(0327)



(Note response from server is mandatory for Login and Heartbeat packet)
